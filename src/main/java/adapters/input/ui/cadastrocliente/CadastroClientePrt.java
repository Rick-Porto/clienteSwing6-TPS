...
import services.application.relatorio.GeraRelatorioClienteCtrl;

import javax.swing.*;

import adapters.input.ui.exportacliente.GerarRelatorioClienteJSON;
import adapters.input.ui.exportacliente.GerarRelatorioClientePDF;
...
public class CadastroClientePrt implements ActionListener {
	...
	public CadastroClientePrt(MainWindow window, CadastroClienteView cadastroView, CadastroClientePort cadastroService,
			CEPPort cepService) {
	...
	private void gerarJSONCliente() {
		...
		// JOptionPane.showMessageDialog(view, "Erro na exportação dos
		// clientes","Exportação de Clientes", JOptionPane.ERROR_MESSAGE);
		try {
			var relatorioJSON = new GeraRelatorioClienteCtrl(
					new GerarRelatorioClienteJSON());

			relatorioJSON.executar();

		} catch (Exception e) {
			JOptionPane.showMessageDialog(
					cadastroView,
					"Erro na exportação do JSON",
					"Exportar Clientes",
	...
	private void gerarRelatorioCliente() {
		...
		// JOptionPane.showMessageDialog(view, "Erro na geração do relatório","Relatório
		// de Clientes", JOptionPane.ERROR_MESSAGE);
		try {
			var relatorioPDF = new GeraRelatorioClienteCtrl(
					new GerarRelatorioClientePDF());

			relatorioPDF.executar();

		} catch (Exception e) {
			JOptionPane.showMessageDialog(
					cadastroView,
					"Erro na geração do relatório de clientes",
